  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        if (res.data && res.data.heroImages && res.data.heroImages.length > 0) {
          const apiImages = res.data.heroImages.map((img: string) => `http://localhost:5000${img}`);
          setImages(apiImages);
        }
      } catch (error) {
      ...
      <div className="max-w-7xl 2xl:max-w-[1760px] 2xl:min-h-[calc(100vh-88px)] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-20 relative z-10 pt-24 2xl:pt-10 pb-32 2xl:pb-20 flex flex-col-reverse lg:flex-row items-center gap-10 2xl:gap-24">
        ...
        <div className="w-full lg:w-1/2 2xl:max-w-[820px] text-[#0f172a] mt-10 lg:mt-0">
          ...
          <div className="flex flex-col sm:flex-row gap-3 2xl:gap-4 mb-5 2xl:mb-6">
            ...
            <a href="https://maps.google.com/maps?q=Data+Storage+Solutions,+Shop+S-11+%26+S-12,+2nd+floor,+20th+Main+Rd,+KHB+Colony,+5th+Block,+Koramangala,+Bengaluru,+Karnataka+560095" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 border border-[#004b9b] text-[#004b9b] px-6 2xl:px-8 py-3 2xl:py-4 text-sm 2xl:text-base font-semibold hover:bg-[#004b9b] hover:text-white transition-colors">
              <svg className="w-5 h-5 2xl:w-6 2xl:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
